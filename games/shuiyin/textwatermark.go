...
func (w *Water) New(src, dis, fileName string, typeface []FontInfo) error {
	var subPath string
	subPath = w.Pattern
	dirs, err := createDir(dis, subPath)
	if err != nil {
		return err
	}
	...
	newName := fmt.Sprintf("%s%s.%s", dirs, fileName, str)
	if str == "gif" {
		err = gifFontWater(src, newName, typeface)
	} else {
		err = staticFontWater(src, newName, str, typeface)
	}
	return err
}
...
func loadFont(ttf string) (*truetype.Font, error) {
	var fontBytes []byte
	var err error
	if ttf == "" {
